...
import { addDoc, getDocs, query, where } from "https://www.gstatic.com/firebasejs/12.2.1/firebase-firestore.js";

// ---- Auth: Register (with auto ID) ----
...
async function revealPack(cards, anim=true) {
  ...
  function showCard(i) {
    ...
    if (anim) {
      ...
      card3d.classList.remove("glow");
      // stagger effect: flip then glow
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const meta = document.querySelector("#overlay-meta");
  if (overlay) {
    overlay.classList.add("hidden");
  }
  if (img) img.src = "";
